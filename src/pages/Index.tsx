...
const CATEGORIES = [
  { icon: "Briefcase", label: "Работа", color: "#2563EB", bg: "#EFF6FF", count: "1 240" },
  { icon: "Wrench", label: "Услуги", color: "#10B981", bg: "#ECFDF5", count: "3 870" },
  { icon: "Home", label: "Недвижимость", color: "#F59E0B", bg: "#FFFBEB", count: "890" },
  { icon: "Car", label: "Транспорт", color: "#EF4444", bg: "#FEF2F2", count: "2 100" },
  { icon: "Laptop", label: "IT / Техника", color: "#8B5CF6", bg: "#F5F3FF", count: "670" },
  { icon: "GraduationCap", label: "Образование", color: "#EC4899", bg: "#FDF2F8", count: "430" },
  { icon: "ShoppingBag", label: "Одежда", color: "#14B8A6", bg: "#F0FDFA", count: "1 560" },
  { icon: "Zap", label: "Другое", color: "#64748B", bg: "#F8FAFC", count: "5 000+" },
];

const LISTINGS = [
  {
    id: 1, title: "Senior Frontend Developer — React/TS", price: "от 200 000 ₽", category: "Работа", location: "Москва / Удалённо",
    time: "10 мин назад", image: "💼", badge: "top", views: 842, fav: false,
    desc: "Ищем опытного разработчика в команду стартапа. Стек: React, TypeScript, GraphQL.",
  },
  {
    id: 2, title: "Ремонт квартир под ключ — бригада", price: "от 3 500 ₽/м²", category: "Услуги", location: "Санкт-Петербург",
    time: "30 мин назад", image: "🔨", badge: "vip", views: 1203, fav: true,
    desc: "Профессиональный ремонт квартир. 7 лет на рынке. 300+ объектов сдано.",
  },
  {
    id: 3, title: "2-комн. квартира в ЖК «Маяк»", price: "95 000 ₽/мес", category: "Недвижимость", location: "Казань",
    time: "1 час назад", image: "🏠", badge: null, views: 310, fav: false,
    desc: "Современная квартира 58 м², 12 этаж, вид на Волгу. Мебель, техника.",
  },
  {
    id: 4, title: "BMW X5 2022 — идеальное состояние", price: "7 200 000 ₽", category: "Транспорт", location: "Краснодар",
    time: "2 часа назад", image: "🚗", badge: "new", views: 560, fav: false,
    desc: "Пробег 28 000 км, один хозяин, полный пакет опций. Сервисная книжка.",
  },
  {
    id: 5, title: "Курс по UX/UI Design — с нуля до PRO", price: "12 900 ₽", category: "Образование", location: "Онлайн",
    time: "3 часа назад", image: "🎓", badge: "top", views: 998, fav: true,
    desc: "80 часов видео, 12 живых воркшопов, трудоустройство после курса.",
  },
  {
    id: 6, title: "MacBook Pro 14\" M3 Max — новый", price: "265 000 ₽", category: "IT / Техника", location: "Новосибирск",
    time: "4 часа назад", image: "💻", badge: null, views: 430, fav: false,
    desc: "Запечатанная коробка, чек, гарантия Apple 1 год. Срочно.",
  },
];
...
const PROMO_PLANS = [
  {
    id: "top", label: "ТОП", icon: "TrendingUp", color: "#F59E0B", glow: "rgba(245,158,11,0.08)",
    price: "299 ₽", period: "7 дней",
    perks: ["Объявление в топе списка", "Выделение цветом", "Метка «ТОП»"],
    popular: false,
  },
  {
    id: "vip", label: "VIP", icon: "Crown", color: "#8B5CF6", glow: "rgba(139,92,246,0.08)",
    price: "799 ₽", period: "14 дней",
    perks: ["ТОП + VIP-метка", "Показ в категории", "Рассылка по базе", "Приоритетная поддержка"],
    popular: true,
  },
  {
    id: "turbo", label: "ТУРБО", icon: "Zap", color: "#EF4444", glow: "rgba(239,68,68,0.08)",
    price: "1 490 ₽", period: "30 дней",
    perks: ["VIP + push-уведомления", "Баннер на главной", "Аналитика просмотров", "Менеджер для продвижения"],
    popular: false,
  },
];
          ...
          <nav className="hidden md:flex items-center gap-1">
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center px-3 py-1.5 rounded-xl text-xs font-medium transition-all duration-150 ${
                  section === n.id ? "text-blue-600 bg-blue-50" : "text-slate-500 hover:text-slate-800 hover:bg-slate-50"
                }`}
              >
        ...
        {section === "home" && (
          <div className="space-y-8 animate-fade-up">
              ...
              <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <div className="flex gap-3 mt-4 flex-wrap">
                    <button onClick={() => setSection("listings")} className="bg-white text-blue-700 font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-blue-50 transition-all duration-150 active:scale-95">
                      Смотреть объявления
                    </button>
                    <button onClick={() => setSection("post")} className="bg-blue-500 bg-opacity-40 text-white border border-blue-400 font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-opacity-60 transition-all duration-150 active:scale-95">
                      + Подать бесплатно
                    </button>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Объявлений", value: "15 400+", icon: "FileText", color: "text-blue-600", bg: "bg-blue-50" },
                { label: "Пользователей", value: "42 000", icon: "Users", color: "text-emerald-600", bg: "bg-emerald-50" },
                { label: "Категорий", value: "8", icon: "Grid3X3", color: "text-amber-600", bg: "bg-amber-50" },
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {LISTINGS.slice(0, 3).map((l, i) => (
                  <ListingCard key={l.id} l={l} i={i} favorites={favorites} toggleFav={toggleFav} onBoost={() => setPromoOpen(true)} />
                ))}
              </div>
            </div>
            ...
            >
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="badge-top">ТОП</span>
                </div>
              ...
              <button className="shrink-0 bg-white text-purple-700 font-bold px-6 py-3 rounded-xl text-sm hover:bg-purple-50 transition-all active:scale-95">
                Узнать цены →
              </button>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {CATEGORIES.map((c, i) => (
                <button
                  ...
                  className="bg-white rounded-2xl p-5 border border-slate-100 flex flex-col items-center gap-3 hover:-translate-y-1 hover:shadow-md transition-all duration-200"
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-slate-400 text-xs text-center mt-0.5">{c.count} объявлений</div>
                  </div>
                </button>
              ))}
            </div>
              ...
              {postStep === 1 && (
                <>
                  <div>
                    ...
                    <input className="input-search" placeholder="Например: Разработчик React ищет проект" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea className="input-search min-h-[120px] resize-none" placeholder="Подробно опишите предложение..." />
                  </div>
                  <button onClick={() => setPostStep(2)} className="btn-primary w-full text-center">Далее →</button>
                </>
              )}
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input className="input-search" placeholder="0 ₽" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1.5">Город</label>
                      <input className="input-search" placeholder="Москва" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Фотографии</label>
                    <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-all duration-150">
                      <Icon name="ImagePlus" size={28} className="mx-auto text-slate-300 mb-2" />
                      <p className="text-slate-500 text-sm">Нажмите или перетащите файлы</p>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Телефон</label>
                    <input className="input-search" placeholder="+7 (___) ___-__-__" />
                  </div>
              ...
              {postStep === 3 && (
                <>
                  ...
                  <div className="flex gap-3 mt-2">
                    ...
                    <button className="btn-primary flex-1 text-center" onClick={() => { setPostStep(1); setSection("home"); }}>
                      {selectedPlan ? "Оплатить и опубликовать" : "Опубликовать бесплатно"}
                    </button>
                  </div>
                </>
              )}
          ...
          <div className="animate-fade-up max-w-2xl mx-auto space-y-5">
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-100">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <div className="font-montserrat font-bold text-xl text-slate-900">Иван Петров</div>
                  <div className="text-slate-500 text-sm">ivan@example.com</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="badge-new text-[11px]">Проверен</span>
                    <span className="text-slate-400 text-xs">На AB-GO с 2024 г.</span>
                  </div>
                </div>
                <button className="ml-auto btn-outline text-sm">Редактировать</button>
              </div>
              <div className="grid grid-cols-3 gap-3 pt-4 border-t border-slate-100">
                {[
                  { label: "Объявлений", value: "7", icon: "FileText", color: "text-blue-600" },
                  { label: "Просмотров", value: "3 240", icon: "Eye", color: "text-emerald-600" },
                  { label: "Отзывов", value: "12", icon: "Star", color: "text-amber-500" },
                ].map((s, i) => (
            ...
            <div className="bg-white rounded-2xl border border-slate-100 divide-y divide-slate-100 overflow-hidden">
              {[
                { icon: "FileText", label: "Мои объявления", badge: "7" },
                ...
                { icon: "LogOut", label: "Выйти", badge: null },
              ].map((item, i) => (
                <button key={i} className="w-full flex items-center justify-between px-5 py-4 hover:bg-slate-50 transition-colors duration-100 text-left">
                  <div className="flex items-center gap-3">
                    <Icon name={item.icon} size={20} className={item.label === "Выйти" ? "text-red-500" : "text-slate-500"} />
                    <span className={`font-medium text-sm ${item.label === "Выйти" ? "text-red-500" : "text-slate-800"}`}>{item.label}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {item.badge && <span className="bg-blue-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">{item.badge}</span>}
            ...
            {chatOpen === null ? (
              <div className="bg-white rounded-2xl border border-slate-100 divide-y divide-slate-50 overflow-hidden">
                {CHATS.map((c) => (
                  <button key={c.id} onClick={() => setChatOpen(c.id)} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors text-left">
                    ...
                    {c.unread > 0 && (
                      <span className="bg-blue-600 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center shrink-0">{c.unread}</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 flex flex-col" style={{ height: "520px" }}>
                <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
                  <button onClick={() => setChatOpen(null)} className="text-slate-400 hover:text-slate-700">
                ...
                <div className="flex-1 p-5 space-y-3 overflow-y-auto">
                  <div className="flex justify-start">
                    <div className="bg-slate-100 rounded-2xl rounded-tl-sm px-4 py-2.5 text-sm text-slate-800 max-w-xs">Привет! Ещё актуально объявление?</div>
                  </div>
                  <div className="flex justify-end">
                    <div className="bg-blue-600 rounded-2xl rounded-tr-sm px-4 py-2.5 text-sm text-white max-w-xs">Да, всё актуально! Что вас интересует?</div>
                  </div>
                  <div className="flex justify-start">
                    <div className="bg-slate-100 rounded-2xl rounded-tl-sm px-4 py-2.5 text-sm text-slate-800 max-w-xs">Можете подробнее рассказать об условиях?</div>
                  </div>
                </div>
                <div className="px-4 py-3 border-t border-slate-100 flex gap-3">
                  <input
                    className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-400"
                    placeholder="Написать сообщение..."
                    value={chatMsg}
                    onChange={(e) => setChatMsg(e.target.value)}
                  />
                  <button className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white hover:bg-blue-700 transition-colors active:scale-95">
        ...
        {section === "search" && (
          <div className="animate-fade-up space-y-5">
            ...
            <div className="relative">
              ...
              <input
                autoFocus
                className="input-search pl-12 text-base"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["React разработчик", "Ремонт квартиры", "Курсы английского", "Продажа авто", "Фотограф", "Репетитор"].map((q) => (
                    <button key={q} onClick={() => setSearchQuery(q)} className="bg-white border border-slate-200 text-slate-700 text-sm px-4 py-2 rounded-xl hover:border-blue-400 hover:text-blue-600 transition-all">
                      {q}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {LISTINGS.filter((l) => l.title.toLowerCase().includes(searchQuery.toLowerCase())).length > 0
                  ...
                  : (
                    <div className="col-span-3 text-center py-16 text-slate-400">
                      <Icon name="SearchX" size={40} className="mx-auto mb-3 opacity-40" />
                      <p className="font-medium">Ничего не найдено по запросу «{searchQuery}»</p>
                    </div>
                  )
                }
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setPromoOpen(false)}>
          <div className="bg-white rounded-3xl w-full max-w-2xl p-6 shadow-2xl animate-fade-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setPromoOpen(false)} className="text-slate-400 hover:text-slate-700 p-2">
                <Icon name="X" size={22} />
              </button>
            </div>
            ...
            <button
              ...
              style={{ opacity: selectedPlan ? 1 : 0.5, cursor: selectedPlan ? "pointer" : "not-allowed" }}
            >
          ...
          {[
            ...
            <button
              ...
              className={`flex-1 flex flex-col items-center py-3 gap-0.5 transition-all duration-150 ${
                n.id === "post" ? "relative -top-3" : ""
              } ${section === n.id ? "text-blue-600" : "text-slate-400"}`}
            >
              {n.id === "post" ? (
...
function ListingCard({
  l, i, favorites, toggleFav, onBoost,
}: {
  l: typeof LISTINGS[0];
  i: number;
  favorites: number[];
  ...
  return (
    <div
      className={`bg-white rounded-2xl border overflow-hidden card-hover animate-fade-up cursor-pointer ${
        l.badge === "top" ? "top-glow border-amber-200" : l.badge === "vip" ? "vip-glow border-purple-200" : "border-slate-100"
      }`}
      style={{ animationDelay: `${i * 0.06}s` }}
    >
      <div className="h-36 bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center relative">
        <span className="text-5xl select-none">{l.image}</span>
      ...
      <div className="p-4">
        ...
        <p className="text-slate-400 text-xs line-clamp-2 mb-3">{l.desc}</p>
        <div className="font-montserrat font-bold text-blue-600 text-base mb-3">{l.price}</div>
        ...
        {!l.badge && (
          <button
            ...
            className="mt-3 w-full text-center text-xs text-amber-600 font-semibold bg-amber-50 hover:bg-amber-100 rounded-xl py-2 transition-colors duration-150"
          >
            🔥 Продвинуть в ТОП
          </button>
        )}
...
function PromoPlanCard({
  plan, selected, onSelect, compact = false,
}: {
  plan: typeof PROMO_PLANS[0];
  selected: boolean;
  ...
  return (
    <div
      ...
      className={`relative rounded-2xl border-2 cursor-pointer transition-all duration-200 ${
        selected ? "scale-[1.02] shadow-lg" : "hover:scale-[1.01] hover:shadow-md"
      } ${compact ? "p-3" : "p-5"}`}
      style={{
        borderColor: selected ? plan.color : "#E2E8F0",
        background: selected ? plan.glow : "white",
      }}
    >
      ...
      <div className={`flex ${compact ? "flex-row items-center gap-3" : "flex-col items-center text-center"}`}>
        <div
          className={`rounded-xl flex items-center justify-center shrink-0 ${compact ? "w-9 h-9" : "w-12 h-12 mb-3"}`}
          style={{ background: `${plan.color}20` }}
        >
          <Icon name={plan.icon} size={compact ? 18 : 24} style={{ color: plan.color }} />
        </div>
        <div className={compact ? "flex-1" : ""}>
          <div className="font-montserrat font-bold text-slate-900" style={{ fontSize: compact ? "14px" : "16px" }}>
            {plan.label}
          </div>
          <div className="font-bold" style={{ color: plan.color, fontSize: compact ? "13px" : "22px" }}>
            {plan.price}
            <span className="text-slate-400 font-normal text-xs"> / {plan.period}</span>
          </div>
        </div>
        {!compact && (
          <ul className="text-left mt-3 space-y-1.5 w-full">
            {plan.perks.map((p, i) => (
              <li key={i} className="flex items-start gap-2 text-slate-600 text-xs">
                <Icon name="Check" size={13} className="mt-0.5 shrink-0" style={{ color: plan.color }} />
                {p}
              </li>
            ))}
          </ul>
